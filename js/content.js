/* =============================================
   ...
   Supports localStorage-stored markdown for
   posts and projects (set by admin panel).
   ============================================= */
...
// localStorage key helpers for inline markdown storage
function postMdKey(slug)  { return `hwh_post_${slug}`; }
function projMdKey(id)    { return `hwh_proj_${id}`; }
...
// Fetch post markdown — checks localStorage first, then network
async function fetchPost(slug) {
  // Admin may have stored the .md content in localStorage
  const stored = localStorage.getItem(postMdKey(slug));
  if (stored) return stored;

  // Fall back to fetching from the content directory
...
// Fetch project markdown — checks localStorage first, then network
async function fetchProjectPost(id) {
  // Admin may have stored the .md content in localStorage
  const stored = localStorage.getItem(projMdKey(id));
  if (stored) return stored;
...
function renderBlogItems(blogs, containerId) {
  ...
  container.innerHTML = blogs.map(blog => {
    ...
    // Show a "draft" badge if content is stored in localStorage (not yet in repo)
    const hasDraft = !!localStorage.getItem(postMdKey(blog.slug));
    ...
    <a class="blog-item${blog.pinned ? ' blog-item--pinned' : ''}" href="${postPath}?slug=${blog.slug}">
      ...
      <div class="blog-content">
        <div class="blog-item-tags">
          ...
          ${hasDraft   ? '<span class="blog-tag" style="background:rgba(255,209,102,.12);color:#ffd166;border-color:rgba(255,209,102,.25);">💾 local draft</span>' : ''}
        </div>
